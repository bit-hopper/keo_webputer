...
class TransformModel {
  ...
  pointerMoved(data) {
    ...
    let found = this.editClientViewRecord(
      viewId,
      (record) => (record.lastPointer = pointer),
      true,
    ); // activate if needed
    if (!found) {
      return;
  ...
  viewport(data) {
    ...
    let found = this.editClientViewRecord(
      viewId,
      (record) => (record.lastViewport = scalerRect),
      true,
    ); // activate if needed
    if (!found) {
      return;
      ...
      Object.keys(clientViewRecords).forEach((viewId2) => {
        if (viewId2 !== presenterId) {
          ...
          if (record.isFollowing) {
            this.editClientViewRecord(
              viewId2,
              (rec) => (rec.lastViewport = { ...scalerRect }),
            );
          }
        }
      });
  ...
  startPresentation(requestingId) {
    ...
    if (presenterId) {
      console.warn(
        `${requestingId} can't present while ${presenterId} is presenting`,
      );
      return;
    }
    ...
    let clientViewRecords = this._get("clientViewRecords");
    Object.keys(clientViewRecords).forEach((viewId) => {
      if (viewId !== requestingId)
        this.setFollowing({ viewId, isFollowing: true });
    });
  ...
  stopPresentation(requestingId, force) {
    ...
    if (!force && presenterId !== requestingId) {
      console.warn(
        `rejecting ${requestingId} request to stop presenting; presenter is ${presenterId}`,
      );
      return;
    }
    ...
    Object.keys(clientViewRecords).forEach((viewId) => {
      if (viewId !== requestingId)
        this.setFollowing({ viewId, isFollowing: false });
    });
  ...
  setFollowing(data) {
    ...
    if (isFollowing) {
      ...
      this.editClientViewRecord(
        viewId,
        (record) => (record.isFollowing = false),
      );
    }
  }
...
class TransformView {
  init() {
    ...
    this.subscribe(
      this.sessionId,
      { event: "userAppsChanged", handling: "immediate" },
      "TransformView.userAppsChanged",
    ); // published by PasteUpView.  immediate to allow instant highlighting during a drag operation.
  ...
  setup() {
    ...
    canv.width =
      canv.height =
      this.followCanvasWidth =
      this.followCanvasHeight =
        1000;
  ...
  setRoomName() {
    ...
    if (roomName && window.fromLandingPage) {
      roomName.call(
        "RoomNameView",
        "setName",
        window.fromLandingPage.boardName,
      );
    }
  }
...
`;
  ...
  dblClick(_evt) {
    ...
    this.setScroll(
      viewport.centerX * viewport.scale - visibleRect.width / 2,
      viewport.centerY * viewport.scale - visibleRect.height / 2,
      true,
    );
  }
  ...
  getVisibleClientRect() {
    ...
    if (false) {
      ...
      if (!this.toolsView)
        this.toolsView = window.topView.querySelector("#tools");
      left = this.toolsView.dom.getBoundingClientRect().right;

      if (!this.headerView)
        this.headerView = window.topView.querySelector("#header");
    ...
    if (!this.infoBarView)
      this.infoBarView = window.topView.querySelector("#infoBar");
    let bottom = this.infoBarView.dom.getBoundingClientRect().top;

    if (!this.peersView)
      this.peersView = window.topView.querySelector("#peers");
  ...
  zoomAboutPoint(desiredZoom, fixedClientX, fixedClientY) {
    ...
    if (newZoom !== oldZoom) {
      ...
      translation = {
        x: fixedX * newZoom - fixedClientX,
        y: fixedY * newZoom - fixedClientY,
      };
    }
  ...
  makeFollowMenu(viewIds) {
    ...
    viewIds.forEach((viewId) => {
      ...
      opt.innerHTML =
        this.scaler.call("PasteUpView", "getUserInfo", viewId).nickname ||
        viewId;
      opt.value = viewId;
  ...
  getViewDetails(viewId) {
    ...
    let isFollower =
      presenterId && !isPresenter && !(isLocal && !this.following);
  ...
  joinPresentation(tellModel) {
    ...
    if (tellModel)
      this.publish(this.model.id, "setFollowing", {
        viewId: this.viewId,
        isFollowing: true,
      }); // to model
  ...
  setPresenterString(str, color) {
    if (!this.presenterString) {
      this.presenterString =
        this.dom.parentNode.parentNode.querySelector("#presenterString");
    }
    if (this.presenterString) {
  ...
  pointerDown(evt) {
    ...
    if (evt.target.key !== this.scaler.dom.key) {
      return;
    }
    // if (this.following) {
    //     this.unfollow();
  ...
  wheel(evt) {
    ...
    const now = Date.now();
    if (this.lastWheel && now - this.lastWheel < 33) {
      return;
    }
    this.lastWheel = now;
    ...
    let allowForInertia =
      this.lastWheelBurst && now - this.lastWheelBurst < 2000;
  ...
  updateAllPointers() {
    ...
    Object.keys(viewRecords).forEach((viewId) => {
      ...
      if (record.lastPointer)
        this.pointerMoved({ viewId, ...record.lastPointer });
    });
  }
  ...
  adjustToFollowedViewport(isLocal = false) {
    ...
    let rect = this.dom.getBoundingClientRect();
    let { zoom: newZoom, translation: newTranslation } = this.setViewportToRect(
      scalerRect,
      false,
    ); // don't publish
    ...
    let innerLeft = Math.max(
      0,
      ((scalerRect.x * newZoom - newTranslation.x) / rect.width) * fcWidth,
    );
    let innerTop = Math.max(
      0,
      ((scalerRect.y * newZoom - newTranslation.y) / rect.height) * fcHeight,
    );
    let innerWidth = Math.min(
      fcWidth - innerLeft,
      ((scalerRect.width * newZoom) / rect.width) * fcWidth,
    );
    let innerHeight = Math.min(
      fcHeight - innerTop,
      ((scalerRect.height * newZoom) / rect.height) * fcHeight,
    );
  ...
  setViewportToRect(scalerRect, publish = true) {
    ...
    let desiredZoom = Math.min(
      visibleRect.width / scalerRect.width,
      visibleRect.height / scalerRect.height,
    );
    let newZoom = this.constrainedZoom(desiredZoom);
    ...
    return {
      zoom: newZoom,
      translation: this.setScroll(translationX, translationY, publish),
    };
  }
  ...
  toolButtonPressed(data) {
    ...
    let prevSelected =
      this.toolState && this.toolState.selected ? this.toolState.name : null;
  ...
  setScroll(x, y, publish) {
    ...
    if (x < 0) x = 0;
      ...
      x = Math.min(
        x,
        this.scaler.model._get("boardWidth") * zoom - thisRect.width,
      );
    ...
    if (y < 0) y = 0;
      ...
      y = Math.min(
        y,
        this.scaler.model._get("boardHeight") * zoom - thisRect.height,
      );
    ...
    if (publish) {
      window.topView.throttledInvoke(
        "publishViewport",
        this.scaler.throttleBaseMS * 3,
        () => this.publishViewport(),
      );
    }
  ...
  publishViewport() {
    ...
    this.publish(this.model.id, "viewport", {
      viewId: this.viewId,
      scalerRect: truncated,
    });
  }
  ...
  zoom(z) {
    ...
    let frames = Array.from(this.dom.querySelectorAll("iframe")).map((f) => [
      f,
      f.style.display,
    ]);
...
class CoverView {
  ...
  setPointer() {
    ...
    this.dom.style.setProperty(
      "cursor",
      this.scaler.dom.style.getPropertyValue("cursor"),
    );
  }
  ...
  startRubberBanding(buttonClientRect, appName) {
    ...
    this.ensureScaler();
    let minObjectExtent = this.scaler.model.call(
      "PasteUpModel",
      "getMinObjectExtent",
    );
    this.rubberBandingInfo = {
      minWidth: minObjectExtent.x,
      minHeight: minObjectExtent.y,
      width: defaultWidth,
  ...
  scaleRubberForZoom() {
    ...
    if (this.rubberBandingInfo.dragWidth !== undefined) {
      let { dragWidth, dragHeight, minWidth, minHeight } =
        this.rubberBandingInfo;
  ...
  addMenu(options) {
    ...
    if (placeElement) {
      // optional element that we'll put the menu next to
...
class PasteUpModel {
  ...
  ensureSessionApps() {
    ...
    if (create) {
      ...
      let appDefs = {
        link: {
          label: "web page",
          iconName: "link.svgIcon",
          urlTemplate: "https://keo.computer/?q=${q}",
          order: 10,
          noURLEdit: true,
          noSandbox: true,
          pressHold: {
            appName: "link:secondary",
            label: "custom app",
            iconName: "link.svgIcon",
            urlTemplate: null,
            order: 1,
        ...
        docview: {
          label: "document",
          iconName: "pdf.svgIcon",
          urlTemplate: "https://start.keo.computer/docview/index.html?q=${q}",
          order: 20,
        },
        maps: {
          label: "maps",
          iconName: "map.svgIcon",
          urlTemplate: "https://start.keo.computer/maps/index.html?q=${q}",
          order: 25,
        },
        mediaplayer: {
          label: "media player",
          iconName: "media-player.svgIcon",
          urlTemplate:
            "https://start.keo.computer/media-player/croquet-app.html",
          order: 26,
        },
        text: {
          label: "notes",
          iconName: "text-fields.svgIcon",
          urlTemplate: "https://start.keo.computer/text/apps/?q=${q}",
          order: 40,
        },
        sharescreen: {
          label: "share screen",
          iconName: "share-screen.svgIcon",
          urlTemplate:
            "https://start.keo.computer/share-screen/croquet-app.html?q=${q}",
          order: 60,
        },
        dweb: {
          label: "dweb",
          iconName: "dweb.svgIcon",
          urlTemplate:
            "https://start.keo.computer/dweb-app/croquet-app.html?q=${q}",
          order: 11,
        },
        wallet: {
          label: "wallet",
          iconName: "wallet.svgIcon",
          urlTemplate:
            "https://start.keo.computer/wallet/croquet-app.html?q=${q}",
          order: 12,
        },
        portal: {
          label: "portal",
          iconName: "portal.svgIcon",
          urlTemplate:
            "https://start.keo.computer/portal/croquet-app.html?q=demo-world.html",
          order: 13,
        },
        io: {
          label: "I/O",
          iconName: "io.svgIcon",
          urlTemplate: "https://start.keo.computer/io/croquet-app.html",
          order: 41,
        },
      };
  ...
  chooseNewUserColor(numColors) {
    ...
    let remainingCands = [];
    colorCands.forEach((tally, colorIndex) => {
      if (tally > 0) remainingCands.push(colorIndex);
    });
    // and pick one of those at random.  Replicated random.
    let colorChoice =
      remainingCands[Math.floor(Math.random() * remainingCands.length)];
    return colorChoice;
  }
  setUserInfo(info) {
    ...
    if (viewId.startsWith("viewDomain")) {
      return;
    }
  ...
  moveObjectEdges(info) {
    ...
    let minPadExtent = {
      x: minObjectExtent.x,
      y: minObjectExtent.y - titleHeight,
    };
    ...
    if (left !== undefined) {
      ...
      frameLeft = Math.min(
        left,
        Math.max(targetRight, right || 0) - minPadExtent.x,
      );
    ...
    if (top !== undefined) {
      ...
      frameTop = Math.min(
        top,
        Math.max(targetBottom, bottom || 0) - minPadExtent.y - titleHeight,
      );
      targetTop = frameTop + titleHeight;
      t[5] = frameTop;
      setTransform = true;
  ...
  resizeOrMoveEnd(info) {
    ...
    let width = target
      ? parseFloat(target.style.getPropertyValue("width"))
      : 400;
  ...
  setSessionFavorite(data) {
    ...
    if (!status) {
      ...
      if (existing) {
        let {
          appInfo: { appName: origAppName },
          faveName: origFaveName,
        } = existing;
...
class Workspace {
  ...
  startScripting(info) {
    ...
    let t = frame.getTransform().slice();
    let textFrame = this.newNativeText({
      x: t[4] + w + 10,
      y: t[5],
      width: 400,
      height: 300,
  ...
  savePersistentData() {
    if (this._get("loadingPersistentData")) {
      return;
    }
    if (this._get("loadingPersistentDataErrored")) {
      return;
    }
    ...
    let func = () => {
      ...
      return {
        name,
        version: "3",
        data: top.stringify({ json, sessionFavorites, sessionApps }),
      };
    };
  ...
  setBackground(data) {
    ...
    if (type === "iframe") {
      ...
      // Create a wrapper HTML that will inject the apiKey into the iframe
      let scaler = this.querySelector("#scaler");
      let currentSession = scaler ? scaler._get("currentSession") : null;
      let sessionView = currentSession ? currentSession.view : null;

      // Get the apiKey from multiple sources
      let apiKeyValue = null;
      if (sessionView && sessionView.apiKey) {
        apiKeyValue = sessionView.apiKey;
      } else if (typeof window !== "undefined" && window.apiKey) {
        apiKeyValue = window.apiKey;
      }

      console.log(
        "Creating iframe app with URL:",
        url,
        "apiKey available:",
        !!apiKeyValue,
      );

      // Store apiKey globally for iframe access
      if (apiKeyValue) {
        window._iframeApiKey = apiKeyValue;
      }
      ...
      // After iframe is added, inject apiKey via postMessage when it loads
      if (apiKeyValue) {
        let injectionHandler = () => {
          setTimeout(() => {
            try {
              // Send apiKey to iframe
              let iframeElement = document.getElementById("backdrop-iframe");
              if (iframeElement && iframeElement.contentWindow) {
                ...
                try {
                  iframeElement.contentWindow.apiKey = apiKeyValue;
                } catch (e) {
                  // Cross-origin, this won't work
                }
              }
            } catch (e) {
              console.error("Failed to inject API key to iframe:", e);
            }
          }, 100);
        };
      ...
      // Set up postMessage listener in main window - ALWAYS, not just when apiKeyValue exists
      if (!window._iframeApiKeyListenerSetup) {
        window._iframeApiKeyListenerSetup = true;
        window.addEventListener("message", (event) => {
          if (event.data && event.data.type === "REQUEST_API_KEY") {
            console.log(
              "Received REQUEST_API_KEY from iframe, responding with apiKey:",
              !!window._iframeApiKey,
            );
            event.source.postMessage(
              {
                type: "INJECT_API_KEY",
                apiKey: window._iframeApiKey || "",
              },
              "*",
            );
          }
...
class PasteUpView {
  ...
  setup() {
    ...
    let probablySafari =
      ua.indexOf("Safari") >= 0 && ua.indexOf("Chrome") === -1;
    let size = probablySafari ? 60 : 30;
    let xOff = 24;
    let yOff = probablySafari ? 59 : 29;
    this.model
      .getLibrary("minibrowser.QRIcon")
      .defaultDragImage(size, size)
      .then((img) => {
    ...
    if (launch) {
      ...
      this.loadContents(launch, dataId)
        .then((_loaded) => {
  ...
  setupUserInfo() {
    ...
    this.localUserInfoPromise = new Promise(
      (resolve) => (this.localUserInfoResolver = resolve),
    );
    this.publish(this.model.id, "setUserInfo", {
      nickname,
      initials,
      viewId,
      userId,
      ...
      fetch(url, {
        ...
        .then((json) => {
          if (json) {
            ...
            // Add additional common formats that might not be in the remote list
            ...
            additionalExtensions.forEach((e) => extensions.add(e));
            ...
            console.warn(
              "failed to load PDF-viewer conversion formats, using defaults",
            );
            // Fallback to a reasonable set of common formats
            const defaultTypes = [
              "image/jpeg",
              "image/gif",
              "image/png",
            ...
            resolve({ types: defaultTypes, extensions: defaultExtensions });
          }
        })
        .catch((e) => {
          console.error("Error loading file formats:", e.message);
          // Fallback to a reasonable set of common formats
          const defaultTypes = [
            "image/jpeg",
            "image/gif",
  ...
  pointerMove(evt) {
    ...
    this.call("RemoteCursorView", "localMouseMove", {
      time: evt.timeStamp,
      target,
      x,
      y,
  ...
  openDialog(pos, label, type, callback, initialValue) {
    ...
    if (type === "text") {
      ...
      this.dialog
        ...
        .addEventListener("click", acceptCallback);
    }
  ...
  async handleFileUpload(files, dropPoint = null) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (size > MAX_FILE_MB * 1048576) {
        this.showToastWarning(
          `${file.name} exceeds max size of ${MAX_FILE_MB}MB`,
        );
        continue;
      }
      ...
      let app = await this.getAppForFile(file); // currently assumed to be "image", "docview", or null
    ...
    let makeUrl = (app) =>
      pad.call("TransformView", "urlFromTemplate", appDefs[app].urlTemplate);

    // Handle image uploads - create native images
    if (uploads.image.length) {
      uploads.image.forEach(async (file) => {
        try {
          ...
          this.publish(pad.model.id, "newImage", newImageData);
        } catch (err) {
  ...
  computeSessionHandles() {
    ...
    if (!this.sessionHandlesP) {
      this.sessionHandlesP = new Promise((resolve, reject) => {
        ...
        Promise.all(promises).then(([pHandle, eHandle]) =>
          resolve({ persistent: pHandle, ephemeral: eHandle }),
        );
      });
    }
  ...
  handleSessionInfoRequest(data, source) {
    ...
    let sessionName =
      window.fromLandingPage && window.fromLandingPage.boardName;
    ...
    Promise.all([handles, sessionName]).then(([h, s]) => {
      Croquet.Messenger.send(
        "sessionInfo",
        {
          sessionHandle: h.persistent,
          sessionName: s,
          ephemeralSessionHandle: h.ephemeral,
        },
        source,
      );
    });
  ...
  getAppFavorites(appName) {
    ...
    for (let [url, spec] of Object.entries(sessionFavesDict)) {
      if (spec.appInfo.appName === appName)
        faves.push({ url, sessionName: spec.faveName });
    }
  ...
  setFavorite(spec) {
    ...
    if (userFave !== undefined) {
      ...
      let walletUpdate = {
        favorites: {
          url,
          spec: userFave ? { appInfo, faveName: spec.proposedUserName } : null,
        },
      };
      Croquet.Messenger.send(
        "updateWalletContents",
        walletUpdate,
        this.walletIframe.contentWindow,
      );
    }
  ...
  duplicateAndUpload(newId) {
    return this.save(newId, true).then((dataInfo) => {
      ...
      let location = window.location;
      let newLocation = `${location.origin}${
        location.pathname
      }?r=${newId}&launch=${newId}&dataId=${encodeURIComponent(id)}`;
...
class RemoteCursorView {
  ...
  setPointer() {
    ...
    this.dom.style.setProperty(
      "cursor",
      `${pointer.style.getPropertyValue("background-image")},auto`,
    );
    pointer.remove();
  }
  ...
  localMouseMove(info) {
    ...
    if (this.lastPointer.x !== x || this.lastPointer.y !== y) {
      ...
      window.topView.throttledInvoke(
        ...
        () =>
          this.publishPointer({
            target: this.lastPointer.target,
            x: this.lastPointer.x,
            y: this.lastPointer.y,
            time: this.lastPointer.time,
            viewId: this.viewId,
          }),
      );
    }
  ...
  updatePointerShape(viewId) {
    ...
    let viewDetails = window.topView.pluggableDispatch(
      "viewportTracker",
      "getViewDetails",
      viewId,
    );
...
class AppButtonManager {
  init() {
    ...
    this.subscribe(
      this.sessionId,
      "toolFavoritesChanged",
      "toolFavoritesChanged",
    ); // published by TransformModel
  ...
  refreshAppButtons() {
    ...
    let userApps = scaler.call("PasteUpView", "getUserApps");
    let allApps = {};
    for (let [appName, spec] of Object.entries(sessionApps))
      allApps[appName] = spec;
    for (let [appName, spec] of Object.entries(userApps))
      allApps[appName] = spec;
  ...
  setIconAndLabel(domElement, iconName, label, viewBox, hoverInverted = false) {
    ...
    if (label)
      result.push(`<span class="no-select tool-button-label">${label}</span>`);
  ...
  initUtilityButtonClasses() {
    ...
    this.PresentationButtonClass = this.model.getLibrary(
      "boards.PresentationButton",
    );
  ...
  makeUtilityButton(spec) {
    // spec is { utilityName, label, iconName, buttonClass }
    let ButtonClass = this[spec.buttonClass];
    return new ButtonClass(this, spec);
  }

  doWithAppButtons(fn) {
    Object.values(this.appButtons).forEach(fn);
  }
  updatedToolState(data) {
    this.doWithAppButtons((b) => b.updatedToolState(data));
  }
  dropHighlightedTool(data) {
    this.doWithAppButtons((b) => b.dropHighlightedTool(data));
  }
  toolFavoritesChanged(data) {
    this.doWithAppButtons((b) => b.toolFavoritesChanged(data));
  }
}
...
class VideoChatView {
  init() {
    ...
    if (chat && (chat !== "videoOnly" || chat === "off")) {
      return;
    }
    ...
    if (localVideo) {
      ...
      iframe.src = "../video-chat/index.html";
    }
...
class TextChatView {
  ...
  setup() {
    ...
    let chat = window.fromLandingPage.chat;
    if (chat && (chat !== "textOnly" || chat === "off")) {
      return;
    }
    ...
    this.scaler.call("PasteUpView", "computeSessionHandles").then((handles) => {
      let isLocal =
        window.topView.isRunningLocally() || !window.fromLandingPage
          ? "&isLocal"
          : "";

      let path = window._production;
      iframe.src = this.url(
        `${path}/text-chat.html?q=${handles.persistent}&docked=true${isLocal}`,
      );
...
class ToolsView {
  init() {
    ...
    appRegionDom.addEventListener(
      "dragenter",
      (evt) => this.dragEnter(evt),
      true,
    );
    appRegionDom.addEventListener(
      "dragover",
      (evt) => this.dragOver(evt),
      true,
    );
    appRegionDom.addEventListener(
      "dragleave",
      (evt) => this.dragLeave(evt),
      true,
    );
  ...
  appInfoFromEvent(evt) {
    ...
    let faveType = evt.dataTransfer.types.find((type) =>
      type.includes("x.croquetfave"),
    );
    if (faveType) {
  ...
  dragEnter(evt) {
    ...
    let scaler = window.topView.querySelector("#scaler");
    scaler.call("PasteUpView", "setProvisionalApp", appName, {
      label,
      iconName,
      urlTemplate,
    });
  ...
  drop(evt) {
    ...
    let scaler = window.topView.querySelector("#scaler");
    scaler.call("PasteUpView", "nameAndSetFavorite", pos, "user", {
      url,
      appInfo,
    });
  }
...
class BeaconView {
  ...
  sendBeacon() {
    ...
    if (!window.fromLandingPage.sessionName) {
      return;
    }
    if (!window.Database) {
      return;
...
function p(parent, _json, persistentData) {
  ...
  navHolder.setViewCode([
    "widgets.WheelFilterView",
    "widgets.GestureFilterView",
  ]);
  ...
  recenterButton.call(
    ...
    "beViewButton",
    "Recenter",
    "view-centered.svgIcon",
    "navigation-button navigation-recenter-button",
    "Recenter",
  );
  ...
  zoomInButton.call(
    ...
    "beViewButton",
    "ZoomIn",
    "view-zoom-in.svgIcon",
    "navigation-button navigation-circle-button",
    "Zoom In",
  ); // "ZoomIn" state currently ignored
  ...
  zoomOutButton.call(
    ...
    "beViewButton",
    "ZoomOut",
    "view-zoom-out.svgIcon",
    "navigation-button navigation-circle-button",
    "Zoom Out",
  ); // "ZoomOut" state currently ignored
  ...
  presentationButton.classList.add(
    "navigation-button",
  ...
  toolsLabel.innerHTML =
    '<div class="no-select tools-label"><span>APPS</span><span id="icon">-</span></div>';
...
`;
  ...
  greenlightLogo() {
    // assets/bitmaps/keo_mascot2.png
    return "url('../assets/bitmaps/keo_mascot2.png')";
  }
}
...
export const boards = {
  expanders: [
    ...
    ToolsTabView,
    BeaconView,
    supplemental.TitleView,
    supplemental.MiddleView,
    supplemental.PeerView,
    supplemental.FrameModel,
    supplemental.FrameView,
    supplemental.FrameResizeModel,
    supplemental.FrameResizeView,
    supplemental.FrameMoveView,
    supplemental.FrameMenuModel,
    supplemental.FrameMenuView,
    supplemental.FrameLockModel,
    supplemental.FrameLockView,
    supplemental.FrameTrashModel,
    supplemental.FrameTrashView,
    supplemental.FrameAddressEditModel,
    supplemental.FrameAddressEditView,
    supplemental.RadarModel,
    supplemental.RadarView,
    supplemental.RadarButtonView,
    supplemental.VSeparatorModel,
    supplemental.VSeparatorView,
    supplemental.SeparatorModel,
    supplemental.SeparatorView,
    supplemental.ScaleReadOutModel,
    supplemental.ScaleReadOutView,
    supplemental.PresentationButtonModel,
    supplemental.PresentationButtonView,
    supplemental.VersionStringView,
    supplemental.AnnotationButtonModel,
    supplemental.AnnotationButtonView,
    supplemental.RoomNameModel,
    supplemental.RoomNameView,
    supplemental.RoomParticipantsModel,
    supplemental.RoomParticipantsView,
    ...drawing.expanders,
  ],
  functions: [p],
  classes: [
...
// Set up global message listener for iframe API key requests
// This listens for REQUEST_API_KEY messages from iframes and responds with the API key
// The API key is obtained from window.apiKey which is set by greenlight.js from apiKey.js
if (typeof window !== "undefined") {
  window.addEventListener("message", (event) => {
    if (event.data && event.data.type === "REQUEST_API_KEY") {
      const apiKey = window.apiKey || window._iframeApiKey;
      if (apiKey) {
        console.log(
          "Global listener: Received REQUEST_API_KEY from iframe, responding with apiKey:",
          apiKey.substring ? apiKey.substring(0, 10) + "..." : apiKey,
        );
        event.source.postMessage(
          {
            type: "INJECT_API_KEY",
        ...
        console.warn("⚠️ No API key available to send to iframe");
      }
    }
  });
}
