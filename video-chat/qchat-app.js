...
const sessionConfiguration = {
  channelName:
    searchParams.get("channelName") || searchParams.get("c") || "all",
  nickname: searchParams.get("nickname") || searchParams.get("n") || "",
  initials: searchParams.get("initials") || searchParams.get("i") || "",
  viewColor:
    searchParams.get("viewColor") ||
    searchParams.get("userColor") ||
    searchParams.get("h") ||
    `hsl(${Math.floor(Math.random() * 255)}, 40%, 40%)`,
  mic:
  ...
  appId: window.sessionConfiguration?.appId || "io.croquet.chat",
};
...
class StreamMixerInput {
  ...
  get aspectRatio() {
    return this.video ? this.width / this.height : undefined;
  }
...
class StreamMixer {
  ...
  get filter() {
    return this.canvasContext.filter;
  }
  set filter(filter) {
  ...
  get videoInputs() {
    return this.inputs.filter((input) => input.video);
  }
  ...
  draw() {
    ...
    this.videoInputs.forEach((videoInput) =>
      videoInput.draw(compositingCanvas),
    );
  ...
  get length() {
    return Math.max(100, Math.max(this.width, this.height));
  }
  set length(length) {
      ...
      if (videoInput && !videoInput._fade) {
        ...
        const intervalId = setInterval(() => {
          ...
          if (fadeIn && !isNaN(newAspectRatio) && !isNaN(currentAspectRatio)) {
            const aspectRatio =
              newAspectRatio * interpolation +
              currentAspectRatio * (1 - interpolation);
            this.aspectRatio = aspectRatio;
          }

          if (interpolation < 1) {
            videoInput.alpha = fadeIn ? interpolation : 1 - interpolation;
          } else {
...
class AgoraPeerManager {
  ...
  setUpConnectionPromise() {
    this.connectionP = new Promise(
      (resolve) => (this.resolveConnectionPromise = resolve),
    );
  }
  ...
  get localPeerState() {
    return this.peerState(this.viewId);
  }
  isKnownPeer(viewId) {
  ...
  getPeerIds() {
    return Object.keys(this.peerDict);
  }
  ...
  async setLocalAudio(nativeTrack) {
    ...
    const newAudioTrack = await AgoraRTC.createCustomAudioTrack({
  ...
  async setLocalVideo(nativeTrack) {
    ...
    const state = this.localPeerState;
    const newVideoTrack = await AgoraRTC.createCustomVideoTrack({
      mediaStreamTrack: nativeTrack,
    });
  ...
  registerPeerMedia(viewId, mediaType, track) {
    ...
    if (knownTrack && knownTrack !== track)
      this.chatManager.offPeerMedia(viewId, mediaType, knownTrack);
  ...
  connect() {
    ...
    if (
      this.connectionState === "DISCONNECTED" &&
      this.chatManager.numberOfPeers > 1
    ) {
      ...
      this.client
        .join(this.appID, sessionConfiguration.channelName, null, this.viewId)
        .then((_uid) => {
    ...
    switch (this.connectionState) {
      case "DISCONNECTED":
        ...
        if (localState) {
          ...
          if (localState.audioTrack)
            delete localState.audioTrack._croquetPublished;
          if (localState.videoTrack)
            delete localState.videoTrack._croquetPublished;
  ...
  startCheckingPeerState() {
    this.stopCheckingPeerState();
    this._checkPeersIntervalId = window.setInterval(
      this.checkPeers.bind(this),
      1000,
    );
  }
  ...
  checkPeers() {
    this.getPeerIds().forEach((viewId) => {
      ...
      if (seconds >= 35) {
        console.warn(
          `${viewId} not heard from in ${seconds}s; assuming it has left chat`,
        );
        state.left = true;
  ...
  async ensureAudioMuteState(muted) {
    ...
    if (!audioTrack) return; // audio track not yet initialized
    ...
    if (!muted && !audioTrack._croquetPublished) {
      ...
      if (localState.audioDisabled || localState.audioTrack._croquetPublished)
        return;
  ...
  async ensureVideoMuteState(disabled) {
    ...
    if (!disabled && !videoTrack._croquetPublished) {
      await this.connectionP;
      if (localState.videoDisabled || localState.videoTrack._croquetPublished)
        return;
  ...
  async onUserPublished(user, mediaType) {
    ...
    const tryToSubscribe = async () => {
      ...
      if (status && !errMsg) {
        ...
        if (!track) {
          console.warn(
            `subscribe: ${mediaType} track for ${viewId} disappeared while subscribing`,
          );
          return;
        }
  ...
  async onUserUnpublished(user, mediaType) {
    ...
    if (!track) {
      console.warn(
        `unsubscribe: failed to find ${mediaType} track for ${viewId}`,
      );
  ...
  cleanUpTracksForLeavingPeer(viewId, shutdown = false) {
    ...
    if (audioTrack)
      this.chatManager.offPeerMedia(viewId, "audio", audioTrack, shutdown);
    if (videoTrack)
      this.chatManager.offPeerMedia(viewId, "video", videoTrack, shutdown);
  ...
  onStreamFallback(uid, direction) {
    console.warn(`stream-fallback (${direction} for user ${uid}`);
  }
  onJoinFallbackToProxy(server) {
    console.warn(`join-fallback-to-proxy ${server}`);
  }
  ...
  onNetworkQuality(stats) {
    ...
    if (uplinkNetworkQuality > 1 || downlinkNetworkQuality > 1)
      console.log(stats);
  }
  onException(event) {
    console.log(
      `Agora exception ${event.code} (${event.msg}) for ${event.uid}`,
    );
  }
...
class LocalMediaManager {
  ...
  // PEER INPUT STREAMS
  chatVideoSource() {
    return this.streamMixer.canvasStream.getVideoTracks()[0];
  }
  ...
  setVideoInput() {
    ...
    if (
      videoStream &&
      videoStream._label === selectedLabel &&
      videoStream.active
    ) {
    ...
    const promise = (this._setVideoInputPromise = navigator.mediaDevices
      .getUserMedia({
        video: {
          deviceId: selectedId,
      ...
      .then((stream) => {
        ...
        const newOption = Array.from(videoInputs.options).find(
          (opt) => videoTrack.label === opt.label,
        );
        if (newOption && newOption.value !== selectedId) {
          console.warn(
            `system chose alternate option: "${newOption.label}" cf. requested "${selectedLabel}"`,
          );
        ...
        this.isCameraFacingUser = !videoTrack.label
          .toLowerCase()
          .includes("back");
        if (this.isCameraFacingUser)
          this.elements.localVideoCanvas.dataset.isCameraFacingUser =
            this.isCameraFacingUser;
        else delete this.elements.localVideoCanvas.dataset.isCameraFacingUser;
        ...
        const onMetadata = () => {
          ...
          if (currentStream) {
            this.streamMixer.fadeOut(currentStream).then(() => {
              this.streamMixer.removeStream(currentStream);
              this.stopStream(currentStream);
            });
          }
        ...
        if (videoInput.video.loadedmetadata) onMetadata();
          ...
          videoInput.video.addEventListener("loadedmetadata", onMetadata, {
            once: true,
          });
        }
  ...
  setAudioInput(force) {
    ...
    if (!okToReplace) {
      ...
      startPromise = navigator.mediaDevices.getUserMedia({
    ...
    const promise = (this._setAudioInputPromise = startPromise
      .then((stream) => {
        ...
        const mediaStreamSource =
          this.audioContext.createMediaStreamSource(audioStreamClone);
        mediaStreamSource.connect(this.gainNode);
        this.localInputStreams.mediaStreamSource = mediaStreamSource;
        audioStreamClone.getAudioTracks()[0].onended = () =>
          console.log(`local subsidiary audio track ended unexpectedly`);
  ...
  getAudioLevel(maxAge) {
    ...
    return audioTrack && audioTrack.getAudioLevel
      ? audioTrack.getAudioLevel(maxAge)
      : 0;
  }
  ...
  updateWaveform() {
    ...
    if (config.waveform.length > config.sampleCount) config.waveform.shift();
  }
  stopWaveform() {
    const config = this.waveformConfiguration;
  ...
  drawWaveform(canvas) {
    ...
    waveform.forEach((sample, sampleIndex) => {
      ...
      if (interpolation === 1) {
        context.fillRect(
          targetCenterX - sampleWidth,
          targetCenterY - sampleY,
          sampleWidth * 2,
          sampleHeight,
        );
      } else {
        context.fillRect(
          targetCenterX - sampleX,
          targetCenterY - sampleY,
          sampleWidth,
          sampleHeight,
        );
  ...
  testAudioLevel() {
    ...
    if (
      this.elements.ui.classList.contains("hide-settings") ||
      !this.localInputStreams.audio
    )
      return;
  ...
  get canCaptureAudioStream() {
    return HTMLAudioElement.prototype.captureStream;
  }

  // Local Video Settings
  get frameRate() {
    return this.streamMixer.frameRate;
  }
  ...
  get width() {
    return this.streamMixer.width;
  }
  set width(width) {
    this.streamMixer.width = width;
  }

  get height() {
    return this.streamMixer.height;
  }
  set height(height) {
    this.streamMixer.height = height;
  }

  get compositingCanvas() {
    return this.streamMixer.canvas;
  }
  addOutputCanvas(canvas) {
    this.streamMixer.addOutputCanvas(canvas);
  }
  removeOutputCanvas(canvas) {
    this.streamMixer.removeOutputCanvas(canvas);
  }
...
class ChatManager {
  ...
  initializeUI() {
    ...
    this.elements = {
      ...
      peerRaisingHandTemplate: document.querySelector(
        "#peerRaisingHandTemplate",
      ),
    ...
    this.addEventListener(
      this.elements.toggleAudio,
      "click",
      this.onToggleAudioClick,
    );
    // this.addEventListener(this.elements.toggleAudio, 'mouseover', this.showAudioTooltip);
    this.addEventListener(
      this.elements.toggleMicrophoneTest,
      "click",
      this.onToggleMicrophoneTestClick,
    );

    this.userWantsLocalAudio = sessionConfiguration.mic !== "unavailable";
    if (!this.userWantsLocalAudio)
      this.elements.toggleAudio.classList.add("error");
    ...
    if (isVideoAllowed) {
      ...
      this.addEventListener(
        this.elements.toggleVideo,
        "click",
        this.onToggleVideoClick,
      );
      // this.addEventListener(this.elements.toggleVideo, 'mouseover', this.showVideoTooltip);
      this.addEventListener(
        this.elements.toggleSolo,
        "click",
        this.onToggleSoloClick,
      );
      // this.addEventListener(this.elements.activePeer, 'click', this.onActivePeerClick);
      this.addEventListener(
        this.elements.videoInputs,
        "input",
        this.onVideoInput,
      );

      if (!this.userWantsLocalVideo)
        this.elements.toggleVideo.classList.add("error");

      // ACTIVE PEER
      this.addEventListener(
        this.elements.activePeerVideo,
        "play",
        this.updateActivePeerContainerStyle,
      ); // once the stream is playing, figure out its size
      this.addEventListener(
        this.elements.activePeerVideo,
        "resize",
        this.updateActivePeerContainerStyle,
      ); // watch for changes in size, e.g. on auto-rotate of a mobile device
      // this.addEventListener(this.elements.activePeerMutePeer, 'click', () => this.mutePeer(this.getDisplayedActivePeer()));
      this.addEventListener(
        this.elements.activePeer,
        "click",
        this.onPeerContainerClick,
      );

      // RAISE HAND
      this.addEventListener(
        this.elements.toggleHand,
        "click",
        this.onToggleHandClick,
      );

      // RESIZE OBSERVER
      if (window.ResizeObserver) {
        this.resizeObserverConfiguration = {
          delay: 100,
          timestamp: Date.now(),
        };
  ...
  publishTrackSubscriptions() {
    this.publishToSession("peer-track-subscriptions", {
      viewId: this.viewId,
      subscribed: this.subscribedPeers,
    });
  }
  ...
  async startLocalMedia() {
    ...
    console.warn(
      `starting local media as ${this.viewId}; mic=${sessionConfiguration.mic} video=${sessionConfiguration.video}`,
    );
    ...
    if (audioTrack) {
      if (
        (sessionConfiguration.micSettingInChat || sessionConfiguration.mic) ===
        "on"
      )
    ...
    if (videoTrack) {
      if (
        (sessionConfiguration.videoSettingInChat ||
          sessionConfiguration.video) === "on"
      )
  ...
  getDisplayedActivePeer() {
    return this.elements.activePeer.dataset.viewId || null;
  }
  ...
  isDisplayedActivePeer(viewId) {
    return viewId === this.getDisplayedActivePeer();
  }

  onOrientationChange() {
    this.checkLocalVideoSize();
  }
  ...
  checkLocalVideoSize() {
    ...
    setTimeout(
      () =>
        this.doWithRelevantContainers(this.viewId, (container) =>
          this.updatePeerContainerStyle(container),
        ),
      1000,
    );
  }
        ...
        track._croquetAudioHelper = {
          ...
          getAudioLevel(maxAge) {
            ...
            if (
              maxAge &&
              this.lastLevelTime &&
              Date.now() - this.lastLevelTime <= maxAge
            )
  ...
  onPeerHand({ viewId, raisingHand }) {
    ...
    if (raisingHand) {
      ...
      const handContainer = this.elements.peerRaisingHandTemplate.content
        .cloneNode(true)
        .querySelector(".peerRaisingHand");
      ...
      document
        .querySelectorAll(
          `#peersRaisingHands .peerRaisingHand[data-view-id="${viewId}"]`,
        )
        .forEach((handContainer) => handContainer.remove());
  ...
  _checkPeerIntendedState({ viewId, audioMuted, videoMuted }) {
    ...
    if (audioMuted !== undefined) {
      ...
      if (audioDisabled !== disabled) {
        console.warn(
          `${viewId} audio state mismatch: reporting ${muteStr(disabled)}, but Agora is ${muteStr(audioDisabled)}`,
        );
        mismatch = true;
      }
    }
    ...
    if (videoMuted !== undefined) {
      ...
      if (videoDisabled !== disabled) {
        console.warn(
          `${viewId} video state mismatch: reporting ${muteStr(disabled)}, but Agora is ${muteStr(videoDisabled)}`,
        );
        mismatch = true;
      }
    }
  ...
  onPeerExit(viewId) {
    ...
    if (viewId === this.viewId)
      throw Error("view-exit received for local view");
  ...
  setDefaultActivePeer() {
    ...
    if (numPeers === 1) this.setActivePeer(null);
      ...
      const otherPeerViewId = peerStreamIds.find(
  ...
  setKnownPeers(peerSnapDict) {
    ...
    if (isVideoAllowed) {
      ...
      handsElement
        .querySelectorAll(".peerRaisingHand")
        .forEach((handContainer) => handContainer.remove());
    ...
    peerIds.forEach((vId) =>
      this.setPeerStateFromModel(vId, peerSnapDict[vId]),
    );
  ...
  get numberOfPeers() {
    return this.knownPeerIds().length;
  }
  isPeerKnown(viewId) {
    return !!this.croquetPeerState[viewId];
  }
  peerCombinedState(viewId) {
    return {
      ...this.croquetPeerState[viewId],
      ...this.chatPeerManager.ensurePeerState(viewId),
    };
  }
  get isLocalPeerKnown() {
    return this.isPeerKnown(this.viewId);
  }
  ...
  resizeObserverCallback(entries) {
    ...
    entries.forEach((entry) => {
      ...
      if (timeSinceLastInvocation >= delay) {
        ...
        entry.target.dataset.resizeObserverTimeoutId = window.setTimeout(
          this.resizeObserverCallback.bind(this, [entry]),
          remainingDelay,
        );
      }
    });
  ...
  onResize(_event) {
    ...
    if (timeSinceLastInvocation >= this.resizeConfiguration.delay) {
      ...
      this.resizeConfiguration.timeoutId = window.setTimeout(
  ...
  _onResize() {
    ...
    this.elements.peers
      ...
      .forEach((peerContainer) => this.updatePeerContainerStyle(peerContainer));
  }
  ...
  onToggleMicrophoneTestClick(event) {
    if (event.shiftKey) {
      ...
      this.publishToSession("gather-logs", {
        initiator: this.viewId,
        reason: "debug",
      });
      return;
    }
  ...
  // new user selection
  onAudioInput() {
    if (this.userWantsLocalAudio) this.localMediaManager.setAudioInput();
  }
  onVideoInput() {
    if (this.userWantsLocalVideo) this.localMediaManager.setVideoInput();
  }

  // AUDIO, VIDEO
  toggleAudio() {
    if (
      !this.localMediaManager ||
      !this.isLocalPeerKnown ||
      !this.userWantsLocalAudio
    )
      return;
  ...
  async muteChatAudio(stopStream) {
    console.log(
      `muting local audio; ${stopStream ? "also" : "not"} stopping stream`,
    );
  ...
  muteChatVideo(stopStream) {
    console.log(
      `muting local video; ${stopStream ? "also" : "not"} stopping stream`,
    );
  ...
  startAnnouncingStreamState() {
    this._announceStreamStateIntervalId = window.setInterval(
      this.announceStreamState.bind(this),
      5000,
    );
  }
  ...
  announceStreamState() {
    ...
    if (
      !peerState.published &&
      peerState.lastAnnounce &&
      Date.now() - peerState.lastAnnounce < 14000
    )
      return;

    this.chatPeerManager.setPeerLastAnnounce(this.viewId);
    this.publishToSession("peer-intended-state", {
      viewId: this.viewId,
      audioMuted: this.chatAudioMuted,
      videoMuted: this.chatVideoMuted,
    });
  }
  ...
  onMouseDown(_event) {
    ...
    else this.elements.ui.classList.remove("resizing");
  }
  onMouseMove(event) {
    if (this.elements.ui.classList.contains("solo")) return;
    ...
    if (flexDirection.includes("row")) {
      ...
      else this.elements.ui.classList.remove("resize");
    ...
    if (this.elements.ui.classList.contains("resizing")) {
      if (flexDirection.includes("row")) {
        ...
        let flexBasis =
          (100 * (this.elements.ui.clientHeight - event.clientY)) /
          this.elements.ui.clientHeight;
        this.elements.peers.style.flexBasis = `${flexBasis}%`;
      }
    }
  }
  ...
  onUpdateSolo(solo) {
    ...
    if (solo) {
      ...
      if (!this.isDisplayedActivePeer(this.viewId)) {
        this.localMediaManager.removeOutputCanvas(
          this.elements.localVideoCanvas,
        );
      }
  ...
  appendPeerContainer(viewId) {
    ...
    const peerContainer = this.elements.peerTemplate.content
      .cloneNode(true)
      .querySelector(".peer");
    ...
    if (this.resizeObserver) this.resizeObserver.observe(peerContainer);

    if (this.viewId === viewId) peerContainer.classList.add("self");
  ...
  getPeerContainer(viewId) {
    return this.elements.peers.querySelector(`.peer[data-view-id="${viewId}"]`);
  }
    ...
    if (peerContainer) {
      if (this.viewId === viewId) {
        ...
        if (this.isDisplayedActivePeer(viewId))
          this.setActivePeerVideo(videoTrack);
      }
  ...
  updatePeersContainerStyle() {
    ...
    while (numberOfPeers > rows * columns) {
      ...
      if (aspectRatios.row / aspectRatios.column < 1.33) rows++;
      else columns++;
    }
  ...
  updatePeerDefiningStyle(element, peerState) {
    ...
    if (element !== this.elements.activePeer) element.style.order = peerIndex;
  }
  ...
  updatePeerEphemeralStyle(element, peerState) {
    ...
    if (viewId !== this.viewId) {
      ...
      const incomplete =
        (!this.chatAudioMuted && !subs.includes("a")) ||
        (!this.chatVideoMuted && !subs.includes("v"));
  ...
  updatePeerContainerStyle(peerContainer) {
    ...
    const aspectRatio =
      (viewId && !this.peerCombinedState(viewId).videoDisabled
        ? this.updatePeerContainerVideo(peerContainer)
        : null) || 1.333; // if video isn't ready, assume the most common aspect ratio
    ...
    else peerContainer.classList.remove("wide");
  ...
  startRenderingPeerBorders() {
    ...
    this._renderPeerBordersIntervalId = window.setInterval(
      this.renderPeerBorders.bind(this),
      this._renderPeerBordersInterval,
    );
  }
  renderPeerBorders() {
    ...
    if (this.solo) {
      ...
      peerIds = this.filteredPeerIds(
        (peer) => peer.published && peer.videoDisabled && !peer.audioDisabled,
      );
    }
      ...
      peerIds.forEach((viewId) => {
        ...
        if (audioTrack) {
          const audioLevel = audioTrack
            .getMediaStreamTrack()
            .getAudioLevel(this._renderPeerBordersInterval / 2); // grab existing value if recent enough
          ...
          if (viewId === activePeerId)
            addShadow(
              this.elements.activePeer.querySelector(".peerInfoSizer"),
              audioLevel,
              viewColor,
            );
          ...
          if (!this.solo) {
            ...
            if (peerContainer)
              addShadow(
                peerContainer.querySelector(".peerInfoSizer"),
                audioLevel,
                viewColor,
              );
  ...
  startPollingForActivePeer() {
    this._pollForActivePeerIntervalId = window.setInterval(
      this.pollForActivePeer.bind(this),
      200,
    );
  }
  pollForActivePeer() {
    ...
    if (
      this.isCroquetOffline ||
      document.visibilityState !== "visible" ||
      this.numberOfPeers < 3 ||
      !localPeer.published ||
      (this._lastRequestToBeActivePeer &&
        Date.now() - this._lastRequestToBeActivePeer < 1500) ||
      currentActivePeer === this.viewId
    )
      return;
    ...
    // or iff level is high enough
      ...
      const activePeerIsMuted =
        !currentActivePeer ||
        !this.isPeerKnown(currentActivePeer) ||
        this.peerCombinedState(currentActivePeer).audioDisabled;
      const levelNeeded = activePeerIsMuted ? 0.1 : 0.3;
      const audioLevel =
        Math.round(this.localMediaManager.getAudioLevel() * 100) / 100;
      if (audioLevel > levelNeeded)
        request = `audio level=${audioLevel} > ${levelNeeded}`;
    }
  ...
  setActivePeer(activePeerId) {
    ...
    const peerDescription = activePeerId
      ? ` (${activePeerId === this.viewId ? "me" : "not me"})`
      : "";
    console.log(
      `active peer=${activePeerId}${peerDescription} numPeers=${this.numberOfPeers}`,
    );
    ...
    if (activePeerId) {
      ...
      let outgoingVideoWidth = 480,
        outgoingFrameRate = 12; // unless told otherwise
      ...
      if (activePeerId === this.viewId) {
        ...
        // active peer.
        if (!this.solo) {
          this.localMediaManager.addOutputCanvas(
            this.elements.localVideoCanvas,
          );
        }
      } else {
  ...
  updateHiddenAudience() {
    ...
    const hiddenAudience =
      this.elements.activePeer.querySelector("#hiddenAudience");
    if (hiddenCount > 0) {
      hiddenAudience.classList.add("someHidden");
      hiddenAudience.querySelector("span").textContent =
        `${hiddenCount} hidden`;
    } else {
  ...
  isKnownChatPeer(viewId) {
    return this.chatPeerManager.isKnownPeer(viewId);
  }
  getPeerMedia(viewId, mediaType) {
    return this.chatPeerManager.getPeerMedia(viewId, mediaType);
  }
  ensureAudioMuteState(bool) {
    return this.chatPeerManager.ensureAudioMuteState(bool);
  }
  ensureVideoMuteState(bool) {
    return this.chatPeerManager.ensureVideoMuteState(bool);
  }
  ...
  removeEventListener(element, type, rawListener) {
    const index = this._eventListeners.findIndex(
      (spec) =>
        ...
        spec.type === type &&
        spec.rawListener === rawListener,
    );
    if (index >= 0) {
      element.removeEventListener(
        type,
        this._eventListeners[index].boundListener,
      );
...
class QChatView extends Croquet.View {
  ...
  sendPeerDetails() {
    ...
    if (initials === "")
      initials = sessionConfiguration.initials = viewId.slice(0, 2);
    const rejoining = !!theChatManager?.isCroquetOffline; // coming back after a glitch?
    const agent = window.navigator.userAgent;

    this.publish(this.sessionId, "peer-details", {
      viewId,
      nickname,
      initials,
      viewColor,
  ...
  onPeerDetails(viewId) {
    ...
    if (this.isWaitingForLocalDetails) {
      ...
      // do so now.
      if (!theChatManager) theChatManager = new ChatManager(this.viewId);
      ...
      this.model
        .identifiedPeers()
        .forEach(
          (vId) => (knownPeerDict[vId] = this.model.peerSnapshotForId(vId)),
        );
      ...
      if (isLocalDetails) {
        // In Croquet v2/Multisynq, local details can be received again on reconnection.
        // This is safe to ignore since we've already set up subscriptions.
        // console.warn(
        //   "local details received twice (reconnection)",
        //   this.model.peerSnapshotForId(viewId),
        // );
        return;
      }
  ...
  setUpSubscriptions() {
    this.subscribe(
      this.sessionId,
      "on-peer-intended-state",
      this.onPeerIntendedState,
    );
    ...
    this.subscribe(
      this.sessionId,
      "on-peer-track-subscriptions",
      this.onPeerTrackSubscriptions,
    );
  ...
  onPeerIntendedState(data) {
    theChatManager.onPeerIntendedState(data);
  }
  onPeerExit(data) {
    theChatManager.onPeerExit(data);
  }
  onUpdateActivePeer(data) {
    theChatManager.onUpdateActivePeer(data);
  }
  onUpdateSolo(data) {
    theChatManager.onUpdateSolo(data);
  }
  onPeerTrackSubscriptions(data) {
    theChatManager.onPeerTrackSubscriptions(data);
  }
  onPeerHand(data) {
    theChatManager.onPeerHand(data);
  }
  ...
  async sendLog(
    ...
    this.publish(this.sessionId, "peer-log", {
      timestamp,
      initiator,
      reason,
      viewId,
  ...
  sendPreviousLogs() {
    ...
    const getAllLogs = () => {
      ...
      this.sendLog("auto");
      this.publish(this.sessionId, "gather-logs", {
        initiator: this.viewId,
        reason: "auto",
      });
    };
...
function resumeAudioContextIfNeeded() {
  ...
  // in 'running' state.
  if (
    mainAudioContext.state !== "running" &&
    mainAudioContext.state !== "closed"
  ) {
...
function joinSession() {
  ...
  Croquet.Session.join(joinArgs).then((_session) => {
...
class ParentConnection {
  constructor() {
    ...
    if (sessionConfiguration.requestName) {
      ...
      nameField.addEventListener("keydown", (evt) =>
        this.nameFieldKeydown(evt),
      );
  ...
  onSessionInfo({ sessionHandle, ephemeralSessionHandle }) {
    ...
    sessionConfiguration.channelName =
      sessionHandle.slice(0, 8) + ":" + ephemeralSessionHandle.slice(0, 8);
  ...
  onUserInfo({ nickname, initials, userColor, viewId: userViewId }) {
    if (nickname) sessionConfiguration.nickname = nickname;
  ...
  onVideoChatInitialState({
    ...
    console.log("initial state from parent:", {
      mic,
      video,
      micDeviceLabel,
      cameraDeviceLabel,
    });
    if (mic && !isBackdrop) sessionConfiguration.mic = mic;

    if (video && !isBackdrop) sessionConfiguration.video = video;

    // fromLandingPage isn't supplied by an in-microverse call requester
    sessionConfiguration.fromLandingPage = fromLandingPage;

    if (cameraDeviceId) sessionConfiguration.cameraDeviceId = cameraDeviceId;

    if (cameraDeviceLabel)
      sessionConfiguration.cameraDeviceLabel = cameraDeviceLabel;

    if (micDeviceId) sessionConfiguration.micDeviceId = micDeviceId;

    if (micDeviceLabel) sessionConfiguration.micDeviceLabel = micDeviceLabel;
  ...
  get receivedAllData() {
    return (
      this.receivedSessionInfo &&
      this.receivedUserInfo &&
      this.receivedVideoChatInitialState
    );
  }
  ...
  checkIfReadyToJoinSession() {
    ...
    if (sessionConfiguration.parentJoinLeave) {
      ...
      if (nickname) {
        ...
        if (window.localStorage) {
          try {
            let settings = JSON.parse(
              window.localStorage.getItem("microverse-settings"),
            );
            ...
            if (oldAvatarURL) {
              ...
              if (predefined) {
                ...
                const avatarURLField =
                  document.getElementById("avatarURLField");
        ...
        console.warn(
          "no nickname supplied, and not configured to ask user to enter one",
        );
  ...
  setJoinDialogFrameSize() {
    let width = 610,
      height = 610; // default, for a wide screen
      ...
      stuff.forEach((arg) => {
        ...
        try {
          ...
          if (argString.length > 500)
            argString =
              argString.slice(0, 500) +
              `...[truncated from ${argString.length} chars]`;
        } catch (e) {
          argString = `[error in logging: ${e}]`;
        }
  ...
  try {
    ...
    for (const [key, value] of Object.entries(localStorage)) {
      ...
      if (now - date > 60 * 60 * 1000)
        delete localStorage[key]; // delete outdated logs
