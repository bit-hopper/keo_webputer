...
function greenlightLoader(dirPath) {
  return new Promise((resolve, _reject) => {
    if (document.querySelector("#greenlightLoader")) {
      // Script already loaded, get the functions from window
      let loadGreenlight = window._loadGreenlight;
      let setPath = window._setPath;
      return resolve({ loadGreenlight, setPath });
    }
    ...
    script.onerror = () => {
      throw new Error("loader could not be loaded");
    };
    return script;
  });
}
...
function openSettingsMenu() {
  ...
  elements.settingsMenu.style.setProperty(
    "left",
    rect.right - sRect.width - 20 + "px",
  );
...
function join() {
  ...
  let cameraDeviceIndex = lookupDeviceInfoIndex(
    videoSelection.current.deviceId,
  );
